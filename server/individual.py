...
from routes import LIST_ALL_INDIVIDUAL
from routes import GET_HALL_OF_FAME
from routes import UPDATE_INDIVIDUAL_TOTAL_MILEAGE
from routes import ACTIVITIES_URL
...
def update_individual_total_mileage():
    athlete_id = request.args.get('id') # athlete ID
    new_mileage = get_total_mileage_from_strava(athlete_id)
    # update DB
    return str(new_mileage)

def get_total_mileage_from_strava(athlete_id):
